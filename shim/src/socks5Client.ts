...
function readExactly(sock: TLSSocket, n: number): Promise<Buffer> {
...
export async function socks5UpstreamConnect(
  up: TLSSocket,
  host: string,
  port: number
): Promise<void> {
  ...
  try {
    ...
    if (greet[0] !== 0x05 || greet[1] !== 0x00) {
      throw new Error(
        `upstream refused method, ver=${greet[0]} method=${greet[1]}`
      );
    }
    ...
    const req: Buffer[] = [Buffer.from([0x05, 0x01, 0x00])];
    if (isIPv4) {
      const parts = host.split(".").map(Number);
      req.push(Buffer.from([0x01, parts[0], parts[1], parts[2], parts[3]]));
    } else if (isIPv6) {
      throw new Error(
        "IPv6 literal not supported in upstream client (use domain)"
      );
    } else {
      const name = Buffer.from(host, "utf8");
      if (name.length > 255) throw new Error("hostname too long");
    ...
    if (rep !== 0x00) throw new Error(`upstream CONNECT failed, REP=${rep}`);

    let toRead = 0;
    if (head[3] === 0x01) toRead = 4;
    else if (head[3] === 0x03) {
      const l = (await readExactly(up, 1))[0];
      toRead = l;
    } else if (head[3] === 0x04) toRead = 16;
    if (toRead) await readExactly(up, toRead);
